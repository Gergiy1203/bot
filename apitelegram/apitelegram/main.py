...
import os
from pyrogram.filters import from_scheduled
...
def mess(message):
    log = open("log.text", "w")
    log.write(message.from_user.first_name + ":" + message.text + "\n")
    log.close()
    format = message.from_user.first_name + ":" + message.text + "\n"
    print(format)


def postgresqlserver(message):
    log = open("log.text", "w")
    log.write(message.from_user.first_name + ":" + message.text + "\n")
    log.close()
    format = message.from_user.first_name + ":" + message.text + "\n"
...
async def postgresqlserver(message):
    log = open("log.text", "w")
    log.write(message.from_user.first_name + ":" + message.text + "\n")
    log.close()
    format = message.from_user.first_name + ":" + message.text + "\n"
...
async def get_user(user_id: int, table_name="users_reg"):
    async with Client("my_account", api_id, api_hash) as app:
        await app.send_message("me", "Greetings from **Pyrogram**!")
...
class SecondClass(object):
    ...
    def __str__(self):
        return f"SecondClass({self.update_types})"
...
country = Country.model_validate(response.message.content)
print(country)
...
res = chat(
    model='gpt-oss',
    messages=[{'role': 'user', 'content':'Tell me about your pets.'}],
    format=Petlist.model_json_schema(),
)    

pets = Petlist.model_validate(res.message.content)
print(pets)
